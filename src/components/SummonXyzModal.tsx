...
import { performXyzSummon } from '../lib/summon';
import CardPopover from './CardPopover';

interface SummonXyzModalProps {
  state: GameState;
  player: Player;
...
export default function SummonXyzModal({ state, player, onStateChange, onClose }: SummonXyzModalProps) {
  const [selectedMaterials, setSelectedMaterials] = useState<Set<number>>(new Set());
  const [selectedExtraCardId, setSelectedExtraCardId] = useState<string | null>(null);
  const [keepMaterials, setKeepMaterials] = useState<boolean>(true);
        ...
        <div style={{ marginBottom: '20px', padding: '12px', background: '#f5f5f5', borderRadius: '4px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
            <input
              ...
              onChange={(e) => setKeepMaterials(e.target.checked)}
            />
            <span style={{ fontSize: '14px' }}>
              素材を保持する（{keepMaterials ? '素材として保持' : '素材を墓地へ送る'}）
            </span>
          </label>
        </div>
