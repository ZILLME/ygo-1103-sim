...
#### カード辞書（効果参照）
- **辞書管理**: 左パネルの「辞書管理」ボタンから一括インポート/エクスポート
- **カード情報表示**: カード名をクリック/ホバーで効果要約・発動タイミング・メモを表示
- **メモ追加**: 辞書にないカードでもメモを追加可能（localStorageに永続化）
- **JSON形式**: 辞書はJSON形式でインポート/エクスポート可能

#### デッキトップ可視化（練習モード）
- **トップN枚表示**: 練習モード（相手手札表示）時にデッキトップ3/5/10枚を表示
- **ログ記録**: 「ログに記録」ボタンでデッキトップを見たことを記録
- **自動更新**: ドロー/サーチ/特殊召喚でデッキが減ったら即座に更新

#### デッキ検索モーダル
- **検索機能**: デッキ内のカードを文字列で検索（部分一致）
- **一括移動**: 複数カードを選択して一括移動
- **移動先選択**:
  - 手札（サーチ）
  - モンスターゾーン（特殊召喚）
  ...
  - 除外
  - デッキトップ/ボトム
- **シャッフル要求**: サーチ/特殊召喚後に「要シャッフル」バッジを表示
- **ログ記録**: すべての移動がNotionログに記録
